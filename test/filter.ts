...
// Minimal parenthesisation: grouping parens are only emitted where OData
// operator precedence (`not` > `and` > `or` > comparison/path) requires them.
...
// `and` binds tighter than `or`, so an `and` operand under `or` needs no parens.
testFilter(
	{ $or: [{ $and: [{ a: 1 }, { b: 2 }] }, { c: 3 }] },
	'a eq 1 and b eq 2 or c eq 3',
);

// ...but an `or` operand under `and` does need parens.
testFilter(
	{ $and: [{ $or: [{ a: 1 }, { b: 2 }] }, { c: 3 }] },
