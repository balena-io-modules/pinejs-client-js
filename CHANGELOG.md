...
* Type the bracketJoin separator as precise literals [Otavio Jacobi]
* Avoid not necessary parenthesis on 'or' and 'and' expressions [Otavio Jacobi]
...
* Combine the two iterations for `bracketJoin` into a single iteration [Pagan Gazzard]
* Use for loops in preference to `forEach` [Pagan Gazzard]
...
* Add support for `Set` in place of arrays in `$select` [Pagan Gazzard]
